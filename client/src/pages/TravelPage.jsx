      ...
      if (uniqueId) {
        try {
          ...
          if (data && data.itinerary) {
            ...
            const coordinatesPromises = data.itinerary.flatMap((dayPlan) =>
            ...
            const cleanedLocations = coordinates
            ...
            setMapsData(cleanedLocations);


            setMapZoom(cleanedLocations.length > 3 ? 10 : 14);


            if (cleanedLocations.length > 0) {
              const avgLat =
                cleanedLocations.reduce((sum, p) => sum + p.lat, 0) /
                cleanedLocations.length;

              const avgLng =
                cleanedLocations.reduce((sum, p) => sum + p.lng, 0) /
                cleanedLocations.length;

              setMapCenter({ lat: avgLat, lng: avgLng });
            }
 // Assuming setMapsData is defined to update the map data state
  ...
  const { destinationCity, budget, itinerary } = itineraryData;
        ...
        <div className="container mx-auto px-4 flex justify-between items-center">
          <div>
            <h1 className="text-4xl font-bold">
              Your Trip to {itineraryData.destinationCity}
            </h1>
          ...
          <div className="bg-gray-100 rounded-lg overflow-hidden">
            <div className="h-[500px] flex items-center flex-col justify-center">
              ...
              <span className="ml-4 text-xl text-gray-600">
                Map of {itineraryData.destinationCity}
              </span>
            </div>
          </div>
        </section>
                      ...
                      const coordinates = await Promise.all(
                        ...
                        const cleanedLocations = coordinates
                          ...
                          .map((c, i) => ({
                            lat: c.lat,
                            lng: c.lng,
                            label: places[i] || `Place ${i + 1}`,
                          }));

                        setMapsData(cleanedLocations);

                        if (cleanedLocations.length > 0) {
                          setMapCenter(cleanedLocations[0]);
                          setMapZoom(15);
                        }
