...
def create_prompt(request_data, db: Session):
    destinationCity = request_data["destinationCity"]
    places = db.query(Place).filter(Place.city == destinationCity).all()
    place_info = ""
    if len(places) != 0:
        place_info = "Here are some popular places in {destinationCity}:\n"
        for place in places[:5]:
            place_info += f"- {place.name}: {place.significance}\n"
    prompt_template = """
    Generate a personalized travel itinerary for a trip to {destinationCity} with a budget of {budget}. 
    The traveler is interested in a {travelStyle} vacation and enjoys. 
    They are looking for {accommodationType} accommodations and prefer {transportationType} transportation. 
    The itinerary should include {activities} activities and {cuisineType} dining options. 
...
def generate_itinerary(request_data, db):
    ...
    # ✅ Extract clean JSON from LLM text
    content = ai_response["choices"][0]["message"]["content"]
    parsed_itinerary = extract_json_from_text(content)
